...
public class ContainerBuilderSettings
extends Container {
	...
	public ContainerBuilderSettings(EntityPlayer player, int id) {
		...
		if (base==null) { this.builderData.id = id; }
		else { this.builderData.read(base.getNbt()); }
		if (this.builderData!=null && this.builderData.type<3) {
			...
			for (int i=1; i<10; i++) {
				this.addSlotToContainer(new Slot(this.builderData.inv, i, 8+(i/6)*54, 17+((i<6 ? 0 : -5)+i-1)*24));
			}
		}
	}
	...
	public void save() {
		if (this.builderData==null || this.builderData.type>2) { return; }
	...
	public ItemStack slotClick(int slotId, int dragType, ClickType clickType, EntityPlayer player) {
		if (clickType == ClickType.QUICK_MOVE || clickType == ClickType.QUICK_CRAFT) { return ItemStack.EMPTY; }
		if (slotId >=36) {
