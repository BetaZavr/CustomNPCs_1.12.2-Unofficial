...
public class ContainerChestCustom extends Container {
	...
	public ContainerChestCustom(InventoryPlayer playerInventory, CustomTileEntityChest customChest, EntityPlayer player) {
		...
		if (this.customChest.getSizeInventory() > 45) { // Creative
			...
			if (this.customChest.getSizeInventory() % 9 != 0) { e -= this.customChest.getSizeInventory() % 9; }
			for (int i = 0; i < this.customChest.getSizeInventory(); i++) {
				if (i>=e) { u = (int) (((9.0d - ((double) this.customChest.getSizeInventory() % 9.0d)) / 2.0d) * 18.0d); }
				this.addSlotToContainer(new Slot(this.customChest, i, 8 + u + (i % 9) * 18, 18 + (int) Math.floor((double) i / 9.0d) * 18));
			}
		}
		// Player Inventory
		for (int r = 0; r < 3; ++r) {
	...
	public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (index < this.customChest.inventory.size()) {
				if (!this.mergeItemStack(itemstack1, this.customChest.inventory.size(), this.inventorySlots.size(), true)) { return ItemStack.EMPTY; }
			}
			else if (!this.mergeItemStack(itemstack1, 0, this.customChest.inventory.size(), false)) { return ItemStack.EMPTY; }
