					...
					if (!property.getString().isEmpty() && property.getString().length() != 6) {
						...
						for (String c : property.getStringList()) {
							if (c.length() > 6) {
								...
								String str;
								for (str = Integer.toHexString(color.getRGB()).toUpperCase(); str
										.length() < 6; str = "0" + str) {
								}
								list.add(str);
							} else if (c.length() < 6) {
				...
				if (!field.getType().isArray()) {
					if (!prop.min().isEmpty()) {
						if (classType.equals("int")) {
							property.setMinValue(Integer.valueOf(prop.min()));
						} else if (classType.equals("double")) {
							property.setMinValue(Double.valueOf(prop.min()));
						}
					}
					if (!prop.max().isEmpty()) {
						if (classType.equals("int")) {
							property.setMaxValue(Integer.valueOf(prop.min()));
						} else if (classType.equals("double")) {
							property.setMaxValue(Double.valueOf(prop.min()));
						}
					}
				} else {
					try {
						isContinue = property.getStringList().length == ((Object[]) field.get(null)).length;
					} catch (Exception e) {
					}
				}
				if (isContinue) {
				...
				if (classType.equals("string")) {
					...
					if (object == null) {
						...
						for (int i = 0; i < vs.length; i++) {
							...
							color = new Color(color.getRed(), color.getGreen(), color.getBlue(), 0);
							for (values[i] = Integer.toHexString(color.getRGB()).toUpperCase(); values[i]
									.length() < 6; values[i] = "0" + values[i]) {
							}
						}
					}
				}
			...
			if (field.getType().isArray()) {
				...
				try {
					if (type == Type.STRING) {
						...
						for (int i = 0; i < values.length && i < base.length; i++) {
							try {
								vs[i] = new Color(Integer.valueOf(values[i]));
							} catch (Exception e) {
