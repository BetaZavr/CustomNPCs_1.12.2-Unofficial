...
public class ItemBuilder extends Item implements IPermission {
	...
	public static void cheakStack(ItemStack stack) {
		...
		if (!compound.hasKey("ID", 3) || id < 1) {
			id = 1;
			while (CommonProxy.dataBuilder.containsKey(id)) {
				id++;
			}
		...
		if (!CommonProxy.dataBuilder.containsKey(id)) {
			...
			for (String key : nbt.getKeySet()) {
				compound.setTag(key, nbt.getTag(key));
			}
		}
	...
	public static boolean isBulderItem(BuilderData bd, ItemStack stack) {
		if (bd == null || stack == null || stack.isEmpty()) {
			return false;
		}
		return stack.getItem() instanceof ItemBuilder && stack.hasTagCompound()
				&& stack.getTagCompound().getInteger("ID") == bd.id;
	}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> list, ITooltipFlag flagIn) {
		...
		for (int i = 0; i < 5; i++) {
			...
			if (i == 4 && (!stack.hasTagCompound() || !stack.getTagCompound().hasKey("ID", 3))) {
				return;
			}
		}
		...
		if (builder.type != 4) {
			...
			if (builder.type != 3) {
				list.add(new TextComponentTranslation("info.item.builder." + (builder.type == 2 ? "6" : "5"))
						.getFormattedText());
				list.add(new TextComponentTranslation("info.item.builder.range.0", "" + builder.region[0],
						"" + builder.region[1], "" + builder.region[2]).getFormattedText());
			} else {
				list.add(new TextComponentTranslation("info.item.builder.range.1", "" + builder.region[0],
						"" + builder.region[1], "" + builder.region[2]).getFormattedText());
			}
		} else {
			for (int i = 0; i < 4; i++) {
				list.add(new TextComponentTranslation("info.item.builder.4." + i).getFormattedText());
			}
			list.add(new TextComponentTranslation("info.item.builder.range.1", "" + builder.region[0],
					"" + builder.region[1], "" + builder.region[2]).getFormattedText());
			list.add(new TextComponentTranslation("gui.name").getFormattedText() + ": "
					+ (builder.schematicaName.isEmpty() ? "empty" : builder.schematicaName + ".schematic"));
		}
	}
	...
	public boolean isAllowed(EnumPacketServer e) {
		return e == EnumPacketServer.BuilderSetting || e == EnumPacketServer.OpenBuilder
	...
	public void leftClick(ItemStack stack, EntityPlayerMP player) {
		...
		if (data == null || !stack.hasTagCompound()) {
			return;
		}
		ItemBuilder.cheakStack(stack);
		BuilderData builder = CommonProxy.dataBuilder.get(stack.getTagCompound().getInteger("ID"));
		...
		ItemStack st = new ItemStack(player.world.getBlockState(pos).getBlock(), 1,
				player.world.getBlockState(pos).getBlock().getMetaFromState(player.world.getBlockState(pos)));
		...
		String name = st.getItem().getRegistryName().toString()
				+ (st.getItemDamage() != 0 ? " [" + st.getItemDamage() + "]" : "");
