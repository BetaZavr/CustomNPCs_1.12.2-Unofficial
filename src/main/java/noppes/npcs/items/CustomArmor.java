...
implements ICustomElement {
		...
		if (this.nbtData.hasKey("OBJData", 9)) {
			...
			for (EnumParts part : EnumParts.values()) {
				...
				switch(part) {
					...
					case MOHAWK: { nbt = data.getCompoundTagAt(0); break; }
					case BODY: { nbt = data.getCompoundTagAt(1); break; }
	...
	@Override
	@SideOnly(Side.CLIENT)
	public ModelBiped getArmorModel(EntityLivingBase entity, ItemStack itemStack, EntityEquipmentSlot slot, ModelBiped defModel) {
