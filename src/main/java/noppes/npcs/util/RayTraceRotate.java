...
public class RayTraceRotate {
	...
	public RayTraceRotate(double yaw, double pitch, double radiusXZ, double dist) {
		this.yaw = yaw;
		this.pitch = pitch;
	...
	public void calculate(double dx, double dy, double dz, double mx, double my, double mz) {
		...
		if (this.pitch < -90.0d) { this.pitch = -90.0d; }
		if (this.pitch > 90.0d) { this.pitch = 90.0d; }

    	if (xVal == 0.0d) { this.yaw = dz > mz ? 180.0d : 0.0d; }
    	else if (xVal <= 0.0d) { this.yaw = 90.0d + Math.atan(zVal / xVal) * rad; }
		else { this.yaw = 270.0d + Math.atan(zVal / xVal) * rad; }
    	this.yaw %= 360.0d;
        if (this.yaw < 0.0d) { this.yaw += 360.0d; }
	}
