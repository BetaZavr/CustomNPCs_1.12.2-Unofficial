...
public class EntityAITargetController extends EntityAIBase {
	...
	public void addDamageFromEntity(EntityLivingBase attackingEntity, double damage) {
		...
		if (attackingEntity instanceof EntityPlayer) {
			if (this.npc.faction.isAggressiveToPlayer((EntityPlayer) attackingEntity)) {
				damage *= 1.2d;
			}
		} else if (attackingEntity instanceof EntityNPCInterface) {
			if (this.npc.faction.isAggressiveToNpc((EntityNPCInterface) attackingEntity)) {
				damage *= 1.2d;
			}
		}
	...
	public void startExecuting() {
		...
		for (EntityLivingBase entity : this.map.keySet()) {
			if (entity == null || !entity.isEntityAlive() || entity.world == null
					|| entity.world.provider.getDimension() != this.npc.world.provider.getDimension()) {
				del.add(entity);
				continue;
			}
			...
			if (maxDist == Double.MIN_VALUE || dist > maxDist) {
				maxDist = dist;
			}
			if (minDist == Double.MAX_VALUE || dist < minDist) {
				minDist = dist;
			}
		}
		...
		if (!tempMap.isEmpty()) {
			...
			for (EntityLivingBase entity : tempMap.keySet()) {
				...
				if (entity.getHeldItemMainhand().getItem() instanceof ItemShield
						|| entity.getHeldItemOffhand().getItem() instanceof ItemShield) {
					value *= 1.5d;
				} else if (entity.getHeldItemMainhand().getItem() instanceof ItemBow
						|| entity.getHeldItemOffhand().getItem() instanceof ItemBow) {
					value *= 1.025d;
				}
				if (this.npc.getAttackTarget() != null && this.npc.getAttackTarget().equals(entity)) {
					value *= 1.25d;
				}
		...
		// set target
		if (this.target != null
				&& (this.npc.getAttackTarget() == null || !this.npc.getAttackTarget().equals(this.target))) {
