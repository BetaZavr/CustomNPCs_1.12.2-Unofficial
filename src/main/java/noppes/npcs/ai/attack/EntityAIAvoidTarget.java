...
extends EntityAICustom {
	...
	public EntityAIAvoidTarget(EntityNPCInterface npc) { super(npc); }
	...
	public void updateTask() {
		...
		if (this.inMove) {
			...
			if (xVal == 0.0d) { yaw = (float) (target.posZ > npc.posZ ? 180.0d : 0.0d); }
	    	else if (xVal <= 0.0d) { yaw = (float) (90.0d + Math.atan(zVal / xVal) * rad); }
			else { yaw = (float) (270.0d + Math.atan(zVal / xVal) * rad); }
	    	yaw %= 360.0d;
			...
			Vec3d vec3d3 = vec3d.addVector(vec3d2.x * tacticalRange, vec3d2.y * tacticalRange, vec3d2.z * tacticalRange);
