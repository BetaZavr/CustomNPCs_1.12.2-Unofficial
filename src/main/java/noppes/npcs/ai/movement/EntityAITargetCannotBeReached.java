...
public class EntityAITargetCannotBeReached
extends EntityAIBase {
				...
				for (int x = -i; x <= i; ++x) {
					...
					for (int z = -i; z <= i; ++z) {
						...
						if (this.isOpaque((int) j, (int) k, (int) l) && !this.isOpaque((int) j, (int) k + 1, (int) l)
							...
							Vec3d vec1 = new Vec3d(this.target.posX,
									this.target.posY + this.target.getEyeHeight(), this.target.posZ);
	...
	public void resetTask() {
		...
		if (!this.npc.isInRange(this.target, this.distance)) {
			this.delay = 60;
		}*/
	}
...
//if (this.npc.getName().equals("Test")) { System.out.println("CNPCs: "+this.npc.aiAttackTarget.distance); }
		...
		if (this.target == null || this.npc.isInRange(this.target, this.distance) || !this.npc.canSee(this.target) || this.delay-- > 0) {
			return false;
		}
	...
	public void startExecuting() {
		//this.npc.getNavigator().tryMoveToXYZ(this.shelterX, this.shelterY, this.shelterZ, 1.3d);
	}
