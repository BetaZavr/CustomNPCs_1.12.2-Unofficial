...
public class DealMarkup {
	...
	public Map<ItemStack, Integer> buyItems = Maps.<ItemStack, Integer>newLinkedHashMap(); // ItemStacks required for purchase
	public Map<ItemStack, Integer> sellItems = Maps.<ItemStack, Integer>newLinkedHashMap(); // ItemStacks received upon sale
	public final Map<ItemStack, Boolean> baseHasPlayerItems = Maps.<ItemStack, Boolean>newLinkedHashMap(); // base ItemStacks
	public Map<ItemStack, Boolean> buyHasPlayerItems = Maps.<ItemStack, Boolean>newLinkedHashMap(); // ItemStacks required for purchase
	public Map<ItemStack, Boolean> sellHasPlayerItems = Maps.<ItemStack, Boolean>newLinkedHashMap(); // ItemStacks received upon sale
	public long baseMoney = 0L, buyMoney = 0L, sellMoney = 0L;
	public boolean baseOneOfEach = true, buyOneOfEach = true, sellOneOfEach = true, ignoreNBT = false, ignoreDamage = false;
		...
		if (inventory==null || !inventory.isEmpty()) {
			...
			for (int slot = 0; slot < inventory.getSizeInventory(); slot++) {
				...
				if (!has) {
					...
					if (stack.getCount() > 1) { this.baseOneOfEach = false; }
				}
			}
	...
	public void set(MarkupData md) {
		...
		if (md.buy!=0.0f) { this.reset(true, (100.0f + md.buy * 100.0f) / 100.0f); }
		else { for (ItemStack stack : this.baseItems.keySet()) { this.buyItems.put(stack, this.baseItems.get(stack)); } }
		if (md.sell!=0.0f) { this.reset(false, (100.0f + md.sell * - 100.0f) / 100.0f); }
		else { for (ItemStack stack : this.baseItems.keySet()) { this.sellItems.put(stack, this.baseItems.get(stack)); } }
		if (!this.sellItems.isEmpty() && this.sellItems.size()<=this.baseItems.size() && this.sellMoney>0) {
			int bc = 0, sc = 0;
			for (int c : this.baseItems.values()) { bc += c;}
			for (int c : this.sellItems.values()) { sc += c;}
			float coff = 1.0f;
			if (md.sell!=0.0f) { coff = (100.0f + md.sell * - 100.0f) / 100.0f; }
			if (sc <= this.sellItems.size() || bc * coff < sc) { this.sellItems.clear(); }
		}
	}
	...
	public void reset(boolean isBuy, float coff) {
		if (!this.baseItems.isEmpty()) {
			for (ItemStack stack : this.baseItems.keySet()) {
				...
				if (count<=0) { count = 1; }
				if (isBuy) { this.buyItems.put(stack, count); }
				else { this.sellItems.put(stack, count); }
			}
		}
		for (ItemStack stack : this.buyItems.keySet()) {
			if (this.buyItems.get(stack)>1) {
				if (isBuy) { this.buyOneOfEach = false; }
				else { this.sellOneOfEach = false; }
				break;
			}
		}
		if (isBuy) { this.buyMoney = (long) ((float) this.baseMoney * coff); }
		else { this.sellMoney = (long) ((float) this.baseMoney * coff); }
		if (this.buyMoney<=0 && this.baseMoney > 0) { this.buyMoney = 1; }
		if (this.sellMoney<=0 && this.baseMoney > 0) { this.sellMoney = 1; }
	}
	...
	public void cheak(NonNullList<ItemStack> inventory) {
		...
		for (ItemStack stack : this.baseItems.keySet()) {
			...
			for (int i = 0; i < inventory.size(); ++i) {
				...
				if (NoppesUtilPlayer.compareItems(stack, s, this.ignoreDamage, this.ignoreNBT)) { count += s.getCount(); }
			}
