...
public class BlockChestRenderer<T extends CustomTileEntityChest>
extends TileEntitySpecialRenderer<T> {
	
	private final ModelChest modelChest = new ModelChest();

	public void render(CustomTileEntityChest te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
		if (!te.isChest || te.chestTexture==null) { return; }
		int i = 0;
		if (te.hasWorld()) { i = te.getBlockMetadata(); }
		...
		else { this.bindTexture(te.chestTexture); }
