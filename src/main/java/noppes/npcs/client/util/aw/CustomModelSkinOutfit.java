...
public class CustomModelSkinOutfit
extends ModelBiped {
	...
	public void render(EntityCustomNpc npc, ISkin skin, ModelBiped modelBiped, Object renderData, float scale, List<Boolean> showList) {
		...
		try {
			...
			if ((boolean) awu.hasPaintData.invoke(skin) & (boolean) awu.isShowSkinPaint.invoke(renderData) & awu.getTexturePaintType.invoke(awu.clientProxy) == awu.TEXTURE_REPLACE) {
				...
				if (showList.get(0)) { bipedHead.render(scale); }
			...
			for (int i = 0; i < parts.size(); i++) {
				...
				if (part.getPartType().getRegistryName().equals("armourers:head.base") && showList.get(0)) {
					GL11.glPushMatrix();
					if (((ModelScaleRenderer) modelBiped.bipedHead).config != null) { ((ModelScaleRenderer) modelBiped.bipedHead).postAWRender(scale); }
					else { modelBiped.bipedHead.postRender(scale); }
