...
public class MusicData {
	...
	public MusicData(ISound s, String u, SoundManager manager) {
		...
		if (this.source!=null && this.source.soundBuffer!=null) {
			...
			if (this.name.indexOf("minecraft")==0) { this.millitotal *= 300.0f; }
		}
	}
	...
	public SoundTickEvent createEvent(EntityPlayer player) {
		...
		return new SoundTickEvent((IPlayer<?>) NpcAPI.Instance().getIEntity(player), this.name, this.resource, NpcAPI.Instance().getIPos(pos[0], pos[1], pos[2]), volume, pitch, milliseconds, this.millitotal);
	}
