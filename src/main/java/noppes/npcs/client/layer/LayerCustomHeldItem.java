...
public class LayerCustomHeldItem<T extends EntityLivingBase> extends LayerInterface<T> {
	...
	public void render(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
		...
		if (!mainhand.isEmpty() || !offhand.isEmpty()) {
			...
			if (this.npc.animation.showParts.get(EnumParts.ARM_LEFT)) {
				boolean show = true;
				if (this.model instanceof ModelBipedAlt) { show = ((ModelBipedAlt) this.model).leftStackData.showModel; }
				else if (this.model instanceof ModelNpcAlt) { show = ((ModelNpcAlt) this.model).leftStackData.showModel; }
			...
			if (this.npc.animation.showParts.get(EnumParts.ARM_RIGHT)) {
				boolean show = true;
				if (this.model instanceof ModelBipedAlt) { show = ((ModelBipedAlt) this.model).rightStackData.showModel; }
				else if (this.model instanceof ModelNpcAlt) { show = ((ModelNpcAlt) this.model).rightStackData.showModel; }
					...
					if (distance <= d) {
                        ...
                        try {
                            ...
                            if (skin != null) {
                                ...
                                Object renderData = awu.skinRenderDataConstructor.newInstance(scale, dye,
                                        awu.extraColours, distance, true, true, false, texture);
	...
	@Override
	public void rotate(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch,
			float scale) {
	}
