...
public class LayerEyes<T extends EntityLivingBase> extends LayerInterface<T> {
	...
	private void drawBrows() {
		...
		if (this.playerdata.eyes.browThickness > 0) {
			...
			this.drawRect(-3.0, -5.0f + offsetY, -1.0, -5.0f - thickness + offsetY, this.playerdata.eyes.browColor,
					4.014);
			this.drawRect(1.0, -5.0f + offsetY, 3.0, -5.0f - thickness + offsetY, this.playerdata.eyes.browColor,
					4.014);
		}
	}

	private void drawLeft() {
		if (this.playerdata.eyes.closed || this.playerdata.eyes.pattern == 2 || this.npc.isDead
				|| this.npc.isPlayerSleeping()) {
			return;
		}
	...
	private void drawRight() {
		if (this.playerdata.eyes.closed || this.playerdata.eyes.pattern == 1 || this.npc.isDead
				|| this.npc.isPlayerSleeping()) {
			return;
		}
		this.drawRect(-3.0, -5.0, -1.0, -4.0, 0xFFF6F6F6, 4.01);
	...
	public void render(float par2, float par3, float par4, float par5, float par6, float par7) {
		if (!this.playerdata.eyes.isEnabled() || this.npc.display.getModel() != null
				|| !this.npc.getClass().getSimpleName().equals("EntityCustomNpc")) {
			return;
		}
		...
		GlStateManager.translate(0.0f, (((this.playerdata.eyes.type == 1) ? 1 : 2) - this.playerdata.eyes.eyePos),
				0.0f);
