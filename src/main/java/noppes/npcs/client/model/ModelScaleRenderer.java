...
public class ModelScaleRenderer extends ModelRenderer {
	...
	public void postAWRender(float scale) {
		if (this.config != null) {
			...
			GlStateManager.translate(this.config.offsetBase[0] + this.config.offsetAnimation[0],
					this.config.offsetBase[1] + this.config.offsetAnimation[1],
					this.config.offsetBase[2] + this.config.offsetAnimation[2]);
		}
		this.postRenderAnimRotate(scale);
		if (this.config != null) {
			GlStateManager.scale(this.config.scaleBase[0] * this.config.scaleAnimation[0],
					this.config.scaleBase[1] * this.config.scaleAnimation[1],
					this.config.scaleBase[2] * this.config.scaleAnimation[2]);
		}
	}
	...
	public void setOBJModel(ItemStack stack, EnumParts part2) {
		...
		this.displayOBJList = ModelBuffer.getDisplayList(armor.objModel,
				armor.getMeshNames(part2 != null ? part2 : this.part), map);
	}
