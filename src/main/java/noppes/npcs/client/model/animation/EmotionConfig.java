...
public class EmotionConfig implements IEmotion {
	 ...
	 * 
	 * @param valueType
	 *            - 0:offsetEye, 1:scales 2:skinColor, 3:browColor, 4:eyeColor,
	 *            5:browThickness
	 * @param partialTicks
	 * @param npc
	...
	public float[] getValues(int valueType, float partialTicks, EntityNPCInterface npc) {
		...
		if (this.frames.containsKey(this.frame + 1)) {
			part_1 = (PartEmotion) this.frames.get(this.frame + 1);
		} else {
			npc.animation.stopEmotion();
			return values;
		}
		if (part_0.isDisabled()) {
			part_0 = EmotionConfig.EMPTY_PART;
		}
		if (part_1.isDisabled()) {
			part_1 = EmotionConfig.EMPTY_PART;
		}
