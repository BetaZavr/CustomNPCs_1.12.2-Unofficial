...
public class GuiDialogEdit extends SubGuiInterface
	...
	public void confirmClicked(boolean result, int id) {
		...
		if (id == 0) {
			Client.sendData(EnumPacketServer.DialogMinID, this.dialog.id);
		}
	}

	@Override
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		if (this.getButton(17) != null) {
			this.getButton(17).setEnabled(this.getTextField(2) != null && !this.getTextField(2).getText().isEmpty());
		}
	...
	public void initGui() {
		...
		if (this.dialog.hasQuest()) {
			this.getButton(7).setDisplayText(this.dialog.getQuest().getTitle());
		}
		...
		if (!this.dialog.mail.title.isEmpty()) {
			this.getButton(13).setDisplayText(this.dialog.mail.title);
		}
