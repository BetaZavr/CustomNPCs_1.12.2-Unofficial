...
implements IGuiData, ICustomScrollListener, GuiYesNoCallback, ISubGuiListener {
	...
	public void initGui() {
		...
		if (scrollMarcets == null) { (scrollMarcets = new GuiCustomScroll(this, 0)).setSize(w, h); }
				...
				if (!marcet.isValid()) {
					...
					for (MarcetSection ms : selectedMarcet.sections.values()) {
						for (Deal deal : ms.deals) {
							...
							if (deal.getProduct().getMCItemStack() == null || deal.getProduct().getMCItemStack().getItem()==Items.AIR) { info.add(new TextComponentTranslation("market.hover.nv.market.deal.0", ""+deal.getId()).getFormattedText()); }
							if (deal.getMoney() == 0 && deal.getCurrency().isEmpty()) { info.add(new TextComponentTranslation("market.hover.nv.market.deal.1", ""+deal.getId()).getFormattedText()); }
						}
					}
		...
		else { scrollMarcets.hoversTexts = null; }
			...
			for (String key : dataDeals.keySet()) {
				...
				if (deal == null || !deal.isValid()) {
					...
					if (tab == tabSelect) {
						...
						marcetInfo.add(((char) 167) + "7" + new TextComponentTranslation("gui.sections").getFormattedText() + " ID: "+((char) 167) + "r" + tab + ((char) 167) + "7; " + new TextComponentTranslation("gui.name").getFormattedText() + ((char) 167) + "7: " + ((char) 167) + "r" + section);
					}
					stack = dm.main;
					...
					marcetInfo.add(dm.main.getDisplayName() + " x" + dm.count + (deal.getMaxCount() > 0 ? " " + new TextComponentTranslation("market.hover.item.amount", new Object[] { "" + deal.getAmount() }).getFormattedText() : ""));
		...
		getButton(7).setEnabled(selectedMarcet != null && selectedMarcet.getDeal(dealId) == null && scrollAllDeals.hasSelected() && selectedDeal != null && selectedDeal.isValid());
	...
	public void buttonEvent(GuiNpcButton button) {
		switch (button.id) {
			...
			case 1: { // Del market
				GuiYesNo guiyesno = new GuiYesNo((GuiYesNoCallback) this, scrollMarcets.getSelected(), new TextComponentTranslation("gui.deleteMessage").getFormattedText(), 0);
			...
			case 4: { // Del deal
				GuiYesNo guiyesno = new GuiYesNo((GuiYesNoCallback) this, scrollDeals.getSelected(), new TextComponentTranslation("gui.deleteMessage").getFormattedText(), 1);
				displayGuiScreen((GuiScreen) guiyesno);
				break;
			}
			case 5: { // Deal settings
				NoppesUtil.requestOpenGUI(EnumGuiType.SetupTraderDeal, marcetId, dataDeals.get(scrollDeals.getSelected()), 0);
				close();
				break;
			}
			case 6: { // tab
	...
	public void scrollClicked(int mouseX, int mouseY, int time, GuiCustomScroll scroll) {
		switch (scroll.id) {
			case 0: { // Marcets
				if (!dataMarcets.containsKey(scroll.getSelected())) { return; }
