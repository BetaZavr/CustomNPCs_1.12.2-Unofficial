...
public class GuiDropEdit extends GuiContainerNPCInterface implements ICustomScrollListener, ITextfieldListener {
	...
	protected GuiCustomScroll scrollAttributes;
	protected GuiCustomScroll scrollEnchants;
	protected GuiCustomScroll scrollTags;
	...
	public GuiDropEdit(EntityNPCInterface npc, ContainerNPCDropSetup cont, GuiContainer gui, int dropTypeIn, int groupIdIn, int pos) {
		super(npc, cont);
	...
	public void buttonEvent(@Nonnull GuiNpcButton button, int mouseButton) {
		...
		switch (button.getID()) {
			...
			case 12: setSubGui(new SubGuiQuestSelection(drop.questId)); break; // quest select
		}
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (getButton(0) != null) {
			...
			if (button.enabled && stack.isEmpty()) { drop.item = Objects.requireNonNull(NpcAPI.Instance()).getIItemStack(stack); }
			else if (!button.enabled && !stack.isEmpty()) { drop.item = Objects.requireNonNull(NpcAPI.Instance()).getIItemStack(stack); }
		}
	...
	public void initGui() {
		super.initGui();
		if (parent == null) { onClosed(); return; }
		int lId = 0;
		// slot
		addLabel(new GuiNpcLabel(lId++, "drop.slot", guiLeft + 171, guiTop + 139)
				.setHoverText("drop.hover.slot"));
		// chance
		addLabel(new GuiNpcLabel(lId++, "drop.chance", guiLeft + 229, guiTop + 145));
		addTextField(new GuiNpcTextField(0, this, guiLeft + 268, guiTop + 140, 50, 20, String.valueOf(drop.getChance()))
		...
		addLabel(new GuiNpcLabel(lId++, "drop.amount", guiLeft + 329, guiTop + 145));
		addTextField(new GuiNpcTextField(1, this, guiLeft + 366, guiTop + 136, 50, 14, "" + amount[0])
				.setMinMaxDefault(1, drop.item.getMaxStackSize(), drop.item.getStackSize())
				.setIsEnable(!drop.item.isEmpty()));
		addTextField(new GuiNpcTextField(2, this, guiLeft + 366, guiTop + 150, 50, 14, "" + amount[1])
				.setMinMaxDefault(1, drop.item.getMaxStackSize(), drop.item.getStackSize())
				.setIsEnable(!drop.item.isEmpty()));
		// damage
		addLabel(new GuiNpcLabel(lId++, "drop.break", guiLeft + 229, guiTop + 174));
		addTextField(new GuiNpcTextField(3, this, guiLeft + 268, guiTop + 169, 50, 20, String.valueOf(drop.getDamage()))
				.setMinMaxDoubleDefault(0.0d, 1.0d, drop.getDamage())
				.setIsEnable(drop.item.getMaxItemDamage() != 0));
		// reset
		addButton(new GuiNpcButton(0, guiLeft + 171, guiTop + 169, 48, 20, "remote.reset", !drop.item.isEmpty())
				.setHoverText("drop.hover.reset"));
		// Enchants:
		// List
		addLabel(new GuiNpcLabel(lId++, "drop.enchants", guiLeft + 4, guiTop + 5)
		...
		// lootMode
		addButton(new GuiNpcButton(10, guiLeft + 329, guiTop + 169, 87, 20, new String[] { "stats.normal", "inv.auto", "inv.inventory" }, drop.lootMode)
				.setIsEnable(!drop.item.isEmpty())
				.setHoverText("drop.hover.mode"));
		// tied level
		addButton(new GuiNpcButton(11, guiLeft + 329, guiTop + 192, 87, 20, new String[] { "drop.type.random", "drop.type.level" }, drop.tiedToLevel ? 1 : 0)
				.setIsEnable(!drop.item.isEmpty())
				.setHoverText("drop.hover.tied"));
		// Quest ID
		addLabel(new GuiNpcLabel(lId, "global.quests", guiLeft + 229, guiTop + 197));
        int qid = Math.max(drop.questId, 0);
		addTextField(new GuiNpcTextField(4, this, guiLeft + 268, guiTop + 192, 50, 20, "" + qid)
				.setMinMaxDefault(0, Integer.MAX_VALUE, qid));
	...
	public void save() {
		...
		if (slot == -1) {
			...
			if (drop.getMinAmount() == 1 && drop.getMinAmount() == 1) { drop.setAmount(drop.item.getStackSize(), drop.item.getStackSize()); }
		}
		drop.item.setStackSize(1);
		Client.sendData(EnumPacketServer.MainmenuInvDropSave, dropType, groupId, slot, drop.save());
	}
	...
	public void subGuiClosed(SubGuiInterface subgui) {
		if (subgui instanceof SubGuiDropEnchant) {
			...
			if (((SubGuiQuestSelection) subgui).selectedQuest != null) { drop.questId = ((SubGuiQuestSelection) subgui).selectedQuest.id; }
		}
		initGui();
	}
	...
	public void unFocused(GuiNpcTextField textField) {
		switch (textField.getID()) {
			...
			case 4: drop.questId = textField.getInteger(); break; // quest set
		}
	}
