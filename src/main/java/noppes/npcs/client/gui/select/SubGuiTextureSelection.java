...
public class SubGuiTextureSelection extends SubGuiInterface implements ICustomScrollListener {
	 ...
	 */
	public SubGuiTextureSelection(int idIn, EntityNPCInterface npcIn, @Nonnull String textureIn, String suffixIn, int typeIn) {
		super(idIn, npcIn);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		Gui.drawRect(x - 1, y - 1, x + w + 1, y + 81, dark ?
				new Color(0xFFE0E0E0).getRGB() :
				new Color(0xFF202020).getRGB());
		Gui.drawRect(x, y, x + w, y + 80, dark ?
		...
		for (int u = 0; u < w / g; u++) {
			for (int v = 0; v < 80 / g; v++) {
				if (u % 2 == (v % 2 == 0 ? 1 : 0)) {
					Gui.drawRect(x + u * g, y + v * g, x + u * g + g, y + v * g + g, dark ?
	...
	public void initGui() {
		...
		addButton(button);
		button = new GuiNpcCheckBox(3, guiLeft + 254, guiTop + 5, 15, 15, null, null, dark);
		...
		if (selectDir == null) {
			...
			list.add(0, back);
			scroll.setUnsortedList(list).setSuffixes(suffixes).setColors(colors);
			if (scroll.getSelect() != pos) { scroll.setSelect(pos); }
			scroll.resetRoll();
			char chr = ((char) 167);
	...
	public SubGuiTextureSelection setOffsetX(int posX) {
		offsetX = posX;
		return this;
	}
