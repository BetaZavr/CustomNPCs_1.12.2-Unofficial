...
extends GuiContainerNPCInterface {
	...
	public void initGui() {
		...
		for (int i = 0; i < 3; ++i) {
			...
			this.addButton(new GuiNpcButton(i, x, y += 18, 50, 14, new TextComponentTranslation("follower.hire").getFormattedText()));
		}
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
		...
		for (int slot = 0; slot < this.role.rentalItems.items.size(); ++slot) {
			...
			if (!NoppesUtilServer.IsItemStackNull(itemstack)) {
				int days = 1;
				if (this.role.rates.containsKey(slot)) { days = this.role.rates.get(slot); }
