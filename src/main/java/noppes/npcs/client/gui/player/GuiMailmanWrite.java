...
implements ITextfieldListener, ITextChangeListener, IGuiError, IGuiClose, GuiYesNoCallback {
	...
	public GuiMailmanWrite(ContainerMail container, boolean canEdit, boolean canSend) {
		...
		this.type = 0;
		this.errTick = 0;
		if (GuiMailmanWrite.mail.message.hasKey("pages")) { this.bookPages = GuiMailmanWrite.mail.message.getTagList("pages", 8); }
	...
	public void buttonEvent(GuiNpcButton button) {
		...
		switch (button.id) {
			...
			case 6: { // ransom
				if (GuiMailmanWrite.mail.ransom > 0) { NoppesUtilPlayer.sendData(EnumPlayerPacket.MailRansom, GuiMailmanWrite.mail.timeWhenReceived); }
				else if (GuiMailmanWrite.mail.money > 0) { NoppesUtilPlayer.sendData(EnumPlayerPacket.MailTakeMoney, GuiMailmanWrite.mail.timeWhenReceived); }
				break;
			}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (this.errTick != 0L && this.errTick < this.mc.world.getTotalWorldTime()) {
			this.errTick = 0L;
			if (this.error != null) { this.error.setLabel(null); }
		}
		// Info
		if (!this.canEdit && GuiMailmanWrite.mail.ransom > 0) {
			...
			this.mc.fontRenderer.drawString(new TextComponentTranslation("mailbox.hover.ransom.sell").getFormattedText(), this.guiLeft + 36, this.guiTop + 100, 0xFFFFFFFF);
		}
		...
		if (this.canEdit && this.canSend && this.getButton(0) != null) {
			...
			if (this.type == 0 && !hasMail) { this.type = 5; } // empty
		...
		if (tick>=0) {
			...
			switch(step) {
				...
				case 4: { // slots
					...
					GlStateManager.popMatrix();
					if (tick % 4 == 0) {
						MusicController.Instance.forcePlaySound(SoundCategory.PLAYERS, CustomNpcs.MODID+":mail.slot", (float) this.player.posX, (float) this.player.posY, (float) this.player.posZ, 1.0f, 0.7f + 0.4f * this.rnd.nextFloat());
					}
					if (tick==0) {
						step = 5;
						tick = 0;
				...
				case 7: { // 0 _ simple close/back
					...
					GlStateManager.popMatrix();
					if (tick % 6 == 0) {
						MusicController.Instance.forcePlaySound(SoundCategory.PLAYERS, CustomNpcs.MODID+":mail.slot", (float) this.player.posX, (float) this.player.posY, (float) this.player.posZ, 1.0f, 0.9f + 0.3f * this.rnd.nextFloat());
					}
					if (tick==0) {
				...
				case 10: { // 1 _ send
					...
					// envelope
					GlStateManager.pushMatrix();
					GlStateManager.translate(this.guiLeft + 142.0f - cos * 142.0f, this.guiTop - 10.0f + cos * 160.0f, 1.0f);
					this.mc.renderEngine.bindTexture(mEnvelope);
					this.drawTexturedModalRect(5, 40, 0, 0, 164, 137);
					GlStateManager.popMatrix();
					if (tick==0) {
				...
				case 12: { // 3 _ send
					...
					if (hasStacks) {
						GlStateManager.pushMatrix();
						GlStateManager.translate(this.guiLeft + 183.0f + cos * 300.0f, this.guiTop + 169.0f - cos * 248.0f, 0.0f);
						this.mc.renderEngine.bindTexture(mSbox);
						this.drawTexturedModalRect(0, 0, 74, 0, 74, 54);
						GlStateManager.popMatrix();
					}
				...
				case 13: { // 1 _ delete
					...
					if (tick==0) {
						...
						tick = 16;
						mtick = 15;
						MusicController.Instance.forcePlaySound(SoundCategory.PLAYERS, CustomNpcs.MODID+":mail.delete", (float) this.player.posX, (float) this.player.posY, (float) this.player.posZ, 1.0f, 0.8f + 0.4f * this.rnd.nextFloat());
					}
					break;
				}
				case 14: { // 2 _ delete
				...
				case 16: { // 4 _ delete
					GlStateManager.pushMatrix();
					GlStateManager.translate(this.guiLeft + 137.5f - cos * 300.0f, this.guiTop + 95.125f - cos * 150.0f, 2.0f);
		...
		if (this.getButton(0) != null && this.getButton(0).isMouseOver()) {
			if (!this.canSend) { this.setHoverText(new TextComponentTranslation("mailbox.hover.done").getFormattedText()); } // done
				...
				else { this.setHoverText(new TextComponentTranslation("mailbox.hover.send."+this.type).getFormattedText()); }
			}
			...
			this.setHoverText(new TextComponentTranslation("display.hover.X").getFormattedText());
		} else if ((this.getTextField(0) != null && this.getTextField(0).isMouseOver()) || (this.getTextField(2) != null && this.getTextField(2).isMouseOver())) {
	...
	private void drawPlace(float u, float v, int mouseX, int mouseY, float partialTicks) {
		...
		if (step == 5) {
			...
			for (int slotId = 0; slotId < this.inventorySlots.inventorySlots.size(); slotId++) {
				...
				if (slotId < 4) {
					boolean show = !this.canEdit && !this.canSend ? mail.ransom <= 0 && slot.getHasStack() : this.canEdit;
			...
			GuiNpcTextField tf;
			GuiNpcButton b;
			if (l != null && l.x == 250000) { l.x = x; l.y = y; }
			if (this.canEdit) {
				tf = this.getTextField(!this.canSend ? 2 : 0);
				if (tf != null && tf.x == 250000) { tf.x = x; tf.y = (y += 10); }
			}
			else {
				l = this.getLabel(10);
				if (l != null && l.x == 250000) { l.x = x + 2; l.y = (y += 10); }
			}
			// title
			l = this.getLabel(1);
			if (l != null && l.x == 250000) { l.x = x; l.y = (y += 18); }
			if (this.canEdit) {
				...
				l = this.getLabel(11);
				if (l != null && l.x == 250000) { l.x = x; l.y = (y += 10); }
			}
			// ransom
			if (this.getLabel(7) != null) { this.getLabel(7).backColor = 0; }
			if (this.getLabel(8) != null) { this.getLabel(8).backColor = 0; }
			if (this.getButton(6) != null) {
				this.getButton(6).layerColor = 0;
				if (!this.canEdit && !this.canSend) {
					if (mail.ransom > 0) { this.getButton(6).enabled = this.player.capabilities.isCreativeMode || ClientProxy.playerData.game.getMoney() >= mail.ransom; }
					else { this.getButton(6).enabled = mail.money > 0; }
				}
			}
			if (!this.canEdit) {
				if (GuiMailmanWrite.mail.ransom > 0 || GuiMailmanWrite.mail.money > 0) {
					...
					if (l != null && l.x == 250000) { l.x = x; l.y = y + 18; }
					l = this.getLabel(8);
					if (l != null && l.x == 250000) { l.x = x + 2; l.y = y + 28; }
				}
			}
			l = this.error;
			if (l != null && l.x == 250000) { l.x = x; l.y = this.guiTop + 145; }
			// Moneys
			if (this.canEdit) {
				l = this.getLabel(3);
				if (l != null && l.x == 250000) { l.x = x; l.y = (y += 19) + 4; }
				l = this.getLabel(6);
				if (l != null && l.x == 250000) { l.x = x + 102; l.y = y + 4; }
				GuiNpcTextField tf3 = this.getTextField(3);
				GuiNpcTextField tf4 = this.getTextField(4);
				if (tf3 != null) {
					if (tf3.x == 250000) { tf3.x = x + 48; tf3.y = y; }
					int tfV = tf3.isEmpty() ? (int) tf3.def : tf3.isInteger() ? tf3.getInteger() : 0;
					if (tfV > 0) {
						if (tf4 != null) { tf4.setVisible(false); }
						if (this.getLabel(7) != null) { this.getLabel(7).enabled = false; }
						if (this.getLabel(8) != null) { this.getLabel(8).enabled = false; }
					} else {
						if (tf4 != null) { tf4.setVisible(true); }
						if (this.getLabel(7) != null) { this.getLabel(7).enabled = true; }
						if (this.getLabel(8) != null) { this.getLabel(8).enabled = true; }
					}
				}
				l = this.getLabel(7);
				if (l != null && l.x == 250000) { l.x = x; l.y = (y += 19) + 4; }
				...
				if (l != null && l.x == 250000) { l.x = x + 102; l.y = y + 4; }
				if (tf4 != null) {
					if (tf4.x == 250000) { tf4.x = x + 48; tf4.y = y; }
					int tfV = tf4.isEmpty() ? (int) tf4.def : tf4.isInteger() ? tf4.getInteger() : 0;
					if (tfV > 0) {
						if (tf3 != null) { tf3.setVisible(false); }
						if (this.getLabel(3) != null) { this.getLabel(3).enabled = false; }
						if (this.getLabel(6) != null) { this.getLabel(6).enabled = false; }
					} else {
						if (tf3 != null) { tf3.setVisible(true); }
						if (this.getLabel(3) != null) { this.getLabel(3).enabled = true; }
						if (this.getLabel(6) != null) { this.getLabel(6).enabled = true; }
					}
				}
			}
			x = this.guiLeft + 7;
			...
			if (this.canEdit && !this.canSend) { // dialog/quest add to
				b = this.getButton(0);
				if (b != null && b.x == 250000) { b.x = x + 52; b.y = y; }
			}
			else if (this.canEdit) { // write
				b = this.getButton(0);
				if (b != null && b.x == 250000) { b.x = x + 52; b.y = y; }
			...
			if (!this.canEdit && !this.canSend) { // read -> delete
				if (GuiMailmanWrite.mail.ransom > 0 || GuiMailmanWrite.mail.money > 0) {
					b = this.getButton(6);
					if (b != null && b.x == 250000) { b.x = x + 220; b.y = y - 42; }
				}
				b = this.getButton(4);
				if (b != null && b.x == 250000) { b.x = x; b.y = y; }
				b = this.getButton(7);
				if (b != null && b.x == 250000) { b.x = x + 52; b.y = y; }
			}
			if (!this.canEdit || this.canSend) { // write -> cancel
				b = this.getButton(3);
				if (b != null && b.x == 250000) { b.x = x + 104; b.y = y; }
			}
			...
			if (this.canEdit) {
				if (this.mc.fontRenderer.getWordWrappedHeight(totalText + "" + TextFormatting.BLACK + "_", 152) > 108) {
					if (this.updateCount / 6 % 2 == 0) { drawEnd = "" + TextFormatting.BLACK + "_"; }
					else { drawEnd = "" + TextFormatting.GRAY + "_"; }
				}
				else if (this.mc.fontRenderer.getBidiFlag()) { totalText += "_"; }
				else if (this.updateCount / 6 % 2 == 0) { totalText = totalText + "" + TextFormatting.BLACK + "_"; }
			...
			if (!drawEnd.isEmpty()) { this.mc.fontRenderer.drawString(drawEnd, this.guiLeft + 159, this.guiTop + 127, 0); }
			...
			if (!this.canEdit && !this.canSend && mail.ransom > 0) {
				this.drawGradientRect(this.guiLeft + 11, this.guiTop + 36, this.guiLeft + 164, this.guiTop + 144, 0x40000000, 0x40000000);
				if (this.isMouseHover(mouseX, mouseY, this.guiLeft + 11, this.guiTop + 36, 152, 108)) {
					List<String> list = Lists.newArrayList(new TextComponentTranslation("mailbox.hover.ransom.sell").getFormattedText());
			...
			for (int slotId = 0; slotId < 4; slotId++) {
				...
				if (show) {
					...
					if (!this.canEdit && !this.canSend && mail.ransom > 0) {
						...
						this.drawString(this.mc.fontRenderer, "" + count, 17 - this.mc.fontRenderer.getStringWidth(""+count), 9, 0xFFFFFFFF);
						if (this.isMouseHover(mouseX, mouseY, px, py, 18, 18)) {
							...
							list.addAll(slot.getStack().getTooltip(this.mc.player, this.mc.gameSettings.advancedItemTooltips ? TooltipFlags.ADVANCED : TooltipFlags.NORMAL));
							this.hoverText = list.toArray(new String[list.size()]);
							this.getLabel(7).backColor = 0x80FF0000;
							this.getLabel(8).backColor = 0x80FF0000;
							this.getButton(6).layerColor = 0xFFF00000;
			...
			for (int slotId = 0; slotId < this.inventorySlots.inventorySlots.size(); slotId++) {
				Slot slot = this.inventorySlots.getSlot(slotId);
				if (slot == null || slot.xPos == 250000) { continue; }
				slot.xPos = 250000;
				slot.yPos = 250000;
			}
	...
	private void addString(String str) {
		...
		if (textHeight <= 108) { this.setText(totalText); }
	}
	...
	private void keyTypedInBook(char c, int i) {
		if (step != 5 || GuiNpcTextField.isActive()) { return; }
		switch (c) {
			case '\u0016': {
			...
			default: {
				switch (i) {
					case 14: { // back
						...
						if (s.length() > 0) { this.setText(s.substring(0, s.length() - 1)); }
						return;
					}
					case 28: // enter
					case 156: {
					...
					default: {
						if (ChatAllowedCharacters.isAllowedCharacter(c)) { this.addString(Character.toString(c)); }
						return;
					}
				}
			}
	...
	public void setError(int id, NBTTagCompound data) {
		...
		switch(id) {
			case 0: this.error.setLabel(new TextComponentTranslation("mailbox.error.username").getFormattedText()); break;
			case 1: this.error.setLabel(new TextComponentTranslation("mailbox.error.subject").getFormattedText()); break;
			case 2: this.error.setLabel(new TextComponentTranslation("mailbox.error.yourself").getFormattedText()); break;
			case 3: this.error.setLabel(new TextComponentTranslation("mailbox.error.nomoney").getFormattedText()); break;
		}
	...
	public void unFocused(GuiNpcTextField textField) {
		switch(textField.getId()) {
			case 0: this.username = textField.getText(); break;
