...
extends GuiContainerNPCInterface {
	...
	public void initGui() {
		...
		boolean isOwnwer = !this.cont.bank.isPublic || this.player.capabilities.isCreativeMode || this.player.getName().equals(this.cont.bank.owner);
		...
		if (!this.stack.isEmpty() || this.player.capabilities.isCreativeMode) {
			...
			button.enabled = this.player.capabilities.isCreativeMode ? (this.update ? this.cont.items.getSizeInventory() < cs.maxCeils : true) : this.cont.dataCeil == this.cont.bank.ceilSettings.size();
		...
		if (this.cont.bank.ceilSettings.size()>1) {
			...
			for (int i = 0; i < 5 && (i + this.ceilPos * 5) < this.cont.bank.ceilSettings.size(); i++) {
				tab = new GuiMenuLeftButton(3 + i, this.guiLeft - 8, this.guiTop + 20 + i * 12, ""+(1 + i + this.ceilPos * 5));
				tab.data = i + this.ceilPos * 5;
				tab.height = 12;
				if (i + this.ceilPos * 5 == this.cont.ceil) { tab.active = true; }
				this.addButton(tab);
			}
		}
		if (this.cont.bank.isPublic) {
			if (!this.cont.bank.owner.isEmpty() || this.player.capabilities.isCreativeMode) {
				this.addButton(new GuiNpcButton(9, (this.isMany ? 12 : -4) + (this.width + this.xSize) / 2, this.guiTop - 8, 20, 20, 20, 146, new ResourceLocation("textures/gui/widgets.png")));
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (!this.stack.isEmpty()) {
			...
			this.drawString(this.mc.fontRenderer, "" + this.stack.getCount(), 16 - this.mc.fontRenderer.getStringWidth(""+this.stack.getCount()), 9, 0xFFFFFFFF);
			RenderHelper.disableStandardItemLighting();
			GlStateManager.popMatrix();
			if (this.isMouseHover(mouseX, mouseY, u + slot.xPos + 54, v + slot.yPos - 22, 18, 18)) {
				List<String> list = new ArrayList<String>();
				CeilSettings cs = this.cont.bank.ceilSettings.get(this.cont.ceil);
				String t = new TextComponentTranslation("bank."+(this.update ? "upg" : "tab")+".cost.info", ""+cs.startCeils, ""+cs.maxCeils).getFormattedText();
				if (t.indexOf("<br>")==-1) { list.add(t); }
				else {
					for (String str : t.split("<br>")) { list.add(str); }
				}
				list.addAll(this.stack.getTooltip(this.mc.player, this.mc.gameSettings.advancedItemTooltips ? TooltipFlags.ADVANCED : TooltipFlags.NORMAL));
				this.hoverText = list.toArray(new String[list.size()]);
			}
			GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
		}
		if (this.getButton(0)!=null) { this.getButton(0).enabled = this.player.capabilities.isCreativeMode || (this.cont.dataCeil == this.cont.bank.ceilSettings.size()); }
			...
			if (this.getButton(0)!=null && this.getButton(0).isMouseOver()) {
				...
				for (int i = 3; i < 8; i++) {
					if (this.getButton(i)!=null && this.getButton(i).isMouseOver()) {
						this.setHoverText(new TextComponentTranslation("bank.hover.ceil."+((GuiMenuLeftButton) this.getButton(i)).active, ""+((GuiMenuLeftButton) this.getButton(i)).data).getFormattedText());
					}
				}
			}
		}
	...
	public void buttonEvent(GuiNpcButton button) {
		...
		switch(button.id) {
			...
			case 2: { // down
				if (this.ceilPos >= Math.floor((double) this.cont.bank.ceilSettings.size() / 5.0d)) { return; }
				this.ceilPos++;
				this.initGui();
				break;
			}
			case 9: { // settings
				if (this.cont.bank == null) { return; }
