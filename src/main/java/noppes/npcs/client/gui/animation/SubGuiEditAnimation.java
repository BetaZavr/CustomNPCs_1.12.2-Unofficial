...
implements ISubGuiListener, ISliderListener, ICustomScrollListener, ITextfieldListener, GuiYesNoCallback {
	...
	public void initGui() {
		...
		this.addButton(new GuiButtonBiDirectional(3, x, y, 60, 20, lFrames.toArray(new String[lFrames.size()]), frame.id));
		...
		button.txrH = 24;
		this.addButton(button);
		this.addLabel(new GuiNpcLabel(lId++, new TextComponentTranslation("gui.time").getFormattedText() + ":", x, (y += 23) + 2));
		...
		if (anim.type == AnimationKind.DIES) {
			this.addLabel(new GuiNpcLabel(lId++, new TextComponentTranslation("gui.repeat").getFormattedText() + ":", x, y - 7));
		...
		for (int id : frame.parts.keySet()) {
			...
			switch(id) {
				case 0: key = "model.head"; break;
				case 1: key = "model.larm"; break;
				case 2: key = "model.rarm"; break;
				case 3: key = "model.body"; break;
				case 4: key = "model.lleg"; break;
				case 5: key = "model.rleg"; break;
				default: key = ps.name; break;
			}
			dataParts.put(key, ps);
			lParts.add(key);
		}
		if (this.scrollParts == null) { (this.scrollParts = new GuiCustomScroll(this, 0)).setSize(60, 60); }
		...
		for (int i = 0; i < 3; i++) {
			...
			if (toolType == 1) { m = -5.0d; n = 5.0d; }
			else if (toolType == 1) { m = 0.0d; n = 5.0d; }
			switch(i) {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		for (Entity e : environmentEntitys) { e.ticksExisted = this.npc.ticksExisted; }
		EntityNPCInterface showNPC = this.getDisplayNpc();
		if (showNPC == null) { this.close();}
		// display data
		if (Mouse.isButtonDown(mousePressId)) {
			int x = mouseX - mousePressX;
			int y = mouseY - mousePressY;
			if (x!=0 || y!=0) {
				if (mousePressId==0) { this.displayOffset(x, y); }// LMB
				else if (mousePressId==1) { this.displayRotate(x, y); } // RMB
				mousePressX = mouseX;
				mousePressY = mouseY;
			}
		}
		...
		if (this.subgui == null) {
			...
			GlStateManager.popMatrix();
				...
				GlStateManager.translate(0.0f, 0.0f, 950.0f);
					...
					if (this.dispRot[2]!=0.0f) { GlStateManager.rotate(this.dispRot[2], 1.0f, 0.0f, 0.0f); }
						GlStateManager.pushMatrix();
				...
				this.fontRenderer.drawString(ts, workS - 11 - this.fontRenderer.getStringWidth(ts), 1, GuiNpcAnimation.backColor==0xFF000000 ? 0xFFFFFFFF : 0xFF000000, false);
				ts = (int) this.dispRot[0] + "" + ((char) 176) + "/" + (int) this.dispRot[1] + ((char) 176) + "/" + (int) this.dispRot[2] + ((char) 176);
				this.fontRenderer.drawString(ts, workS - 11 - this.fontRenderer.getStringWidth(ts), workS - 10, GuiNpcAnimation.backColor==0xFF000000 ? 0xFFFFFFFF : 0xFF000000, false);
				ts = (int) this.dispPos[0] + "/" + (int) this.dispPos[1];
				this.fontRenderer.drawString(ts, 11, workS - 10, GuiNpcAnimation.backColor==0xFF000000 ? 0xFFFFFFFF : 0xFF000000, false);
				GlStateManager.popMatrix();
			GlStateManager.popMatrix();
		} else {
			GlStateManager.pushMatrix();
		...
		if (scrollParts != null && scrollParts.hovered) {
			...
			this.setHoverText(new TextComponentTranslation("animation.hover.reset.frame").appendSibling(new TextComponentTranslation("animation.hover.shift.0")).getFormattedText());
		} else if (this.getButton(7)!=null && this.getButton(7).isMouseOver()) {
			this.setHoverText(new TextComponentTranslation("animation.hover.part.add").appendSibling(new TextComponentString("<br>")).appendSibling(new TextComponentTranslation("gui.wip")).getFormattedText());
			...
			this.setHoverText(new TextComponentTranslation("animation.hover.reset.part").appendSibling(new TextComponentTranslation("animation.hover.shift.0")).getFormattedText());
		...
		} else if ((this.getTextField(5)!=null && this.getTextField(5).isMouseOver()) || (this.getSlider(0)!=null && this.getSlider(0).isMouseOver())) {
			this.setHoverText(new TextComponentTranslation("animation.hover."+(this.toolType==0 ? "rotation" : this.toolType==1 ? "offset" : "scale"), "X").getFormattedText());
		} else if ((this.getTextField(6)!=null && this.getTextField(6).isMouseOver()) || (this.getSlider(1)!=null && this.getSlider(1).isMouseOver())) {
			this.setHoverText(new TextComponentTranslation("animation.hover."+(this.toolType==0 ? "rotation" : this.toolType==1 ? "offset" : "scale"), "Y").getFormattedText());
		} else if ((this.getTextField(7)!=null && this.getTextField(7).isMouseOver()) || (this.getSlider(2)!=null && this.getSlider(2).isMouseOver())) {
			this.setHoverText(new TextComponentTranslation("animation.hover."+(this.toolType==0 ? "rotation" : this.toolType==1 ? "offset" : "scale"), "Z").getFormattedText());
		}
	...
	public void buttonEvent(GuiNpcButton button) {
		switch(button.id) {
			case 0: { // block place
				...
				if (this.getButton(16)!=null) { this.getButton(16).setEnabled(blockType != 1); }
				break;
			}
			case 1: { // block size
			...
			case 3: { // select frame
				if (anim == null || !anim.frames.containsKey(button.getValue())) { return; }
			...
			case 5: { // del frame
				...
				GuiYesNo guiyesno = new GuiYesNo((GuiYesNoCallback) this, new TextComponentTranslation("animation.clear.frame", ""+(frame.id+1)).getFormattedText(), new TextComponentTranslation("gui.deleteMessage").getFormattedText(), 0);
				this.displayGuiScreen((GuiScreen) guiyesno);
				break;
			}
			case 6: { // clear frame
				if (frame==null) { return; }
				GuiYesNo guiyesno = new GuiYesNo((GuiYesNoCallback) this, new TextComponentTranslation("animation.clear.frame", ""+(frame.id+1)).getFormattedText(), new TextComponentTranslation("gui.clearMessage").getFormattedText(), GuiScreen.isShiftKeyDown() ? 4 : 1);
				this.displayGuiScreen((GuiScreen) guiyesno);
				break;
			}
			case 7: { // add part
			...
			case 8: { // del part
				if (part==null) { return; }
				GuiYesNo guiyesno = new GuiYesNo((GuiYesNoCallback) this, new TextComponentTranslation("animation.clear.part", ""+(part.id+1), this.scrollParts.getSelected()).getFormattedText(), new TextComponentTranslation("gui.deleteMessage").getFormattedText(), 2);
				this.displayGuiScreen((GuiScreen) guiyesno);
				break;
			}
			case 9: { // clear part
				if (part==null) { return; }
				GuiYesNo guiyesno = new GuiYesNo((GuiYesNoCallback) this, new TextComponentTranslation("animation.clear.part", ""+(part.id+1), this.scrollParts.getSelected()).getFormattedText(), new TextComponentTranslation("gui.clearMessage").getFormattedText(), GuiScreen.isShiftKeyDown() ? 5 : 3);
				this.displayGuiScreen((GuiScreen) guiyesno);
				break;
			}
			...
			case 13: { // reset mesh
				...
				if (this.getButton(14) != null) { this.getButton(14).layerColor = 0xFF1A0C36; }
				if (this.getButton(15) != null) { this.getButton(15).layerColor = 0xFF0C3620; }
				if (this.getButton(16) != null) { this.getButton(16).layerColor = 0xFF35360C; }
				break;
			}
			case 14: { // xz mesh
				if (meshType == 1) {
				...
				if (this.getButton(13) != null) { this.getButton(13).layerColor = 0xFF360C1C; }
				if (this.getButton(15) != null) { this.getButton(15).layerColor = 0xFF0C3620; }
				if (this.getButton(16) != null) { this.getButton(16).layerColor = 0xFF35360C; }
				break;
			}
			...
			case 19: { // reset pos
				for (int i = 0; i < 3; i++) { this.dispPos[i] = 0.0f; }
				break;
			}
			case 20: { // reset rot
				dispRot[0] = 45.0f;
	...
	public void confirmClicked(boolean result, int id) {
		...
		switch(id) {
			...
			case 2: { // remove part
				if (frame==null || part==null || frame.parts.size() <= 6) { return; }
				int f = part.id - 1;
				if (f<0) { f = 0; }
	...
	public void keyTyped(char c, int i) {
		if (this.subgui == null) {
			...
			if (isPressAltAndKey(i, 25)) {
				onlyCurrentPart = !onlyCurrentPart;
				if (this.getButton(21) != null) { this.getButton(21).txrX = onlyCurrentPart ? 144 : 188; }
				playButtonClick();
			}
			// reset scale - Alt + S
	...
	public void scrollClicked(int i, int j, int k, GuiCustomScroll scroll) {
		if (!this.dataParts.containsKey(scroll.getSelected())) { return; }
	...
	public EntityNPCInterface getDisplayNpc() { return this.onlyCurrentPart ? this.npcPart : this.npcAnim; }
	...
	private void displayOffset(int x, int y) {
		for (int i = 0; i < 2; i++) {
			...
			if (this.dispPos[i] > workS * this.dispScale) { this.dispPos[i] = workS * this.dispScale; }
			else if (this.dispPos[i] < -workS * this.dispScale) { this.dispPos[i] = -workS * this.dispScale; }
		}
	}
	...
	private void setEnvironment() {
		...
		if (this.npc == null || this.npc.world == null) { return; }
		offsetY = 0.0f;
		...
		for (int y = -4; y <= 4; y++) {
			for (int x = -4; x <= 4; x++) {
				for (int z = -4; z <= 4; z++) {
					...
					if (this.npc.posY != Math.floor(this.npc.posY)) { yP = Math.ceil(this.npc.posY) + y - 1; }
	...
	private boolean isPressAltAndKey(int key, int id) {
		...
		if (key == 56 || key == 184) { isPress = Keyboard.isKeyDown(id); }
		else if (key == id) { isPress = Keyboard.isKeyDown(56) || Keyboard.isKeyDown(184); }
		if (isPress) {
			waitKey = 30;
			waitKeyID = id;
		}
