...
public class SubGuiDataSend extends SubGuiInterface implements ITextfieldListener {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (this.getTextField(0) != null && this.getTextField(0).isMouseOver()) {
			this.setHoverText(
					new TextComponentTranslation("hover.data.day", "" + this.getTextField(0).max).getFormattedText());
		} else if (this.getTextField(1) != null && this.getTextField(1).isMouseOver()) {
			this.setHoverText(new TextComponentTranslation("hover.data.month", "" + this.getTextField(1).max,
					new TextComponentTranslation("month." + this.month).getFormattedText()).getFormattedText());
		} else if (this.getTextField(2) != null && this.getTextField(2).isMouseOver()) {
			this.setHoverText(new TextComponentTranslation("hover.data.year", "" + this.getTextField(2).min,
					"" + this.getTextField(2).max).getFormattedText());
		}
	...
	public void initGui() {
		...
		this.getTextField(1).setMinMaxDefault(this.year == 2011 ? 11 : 1,
				(this.year == cal.get(Calendar.YEAR) ? cal.get(Calendar.MONTH) + 1 : 12), (this.month + 1));

		this.addTextField(
				new GuiNpcTextField(2, this.parent, this.guiLeft + 118, this.guiTop + 16, 54, 20, "" + this.year));
