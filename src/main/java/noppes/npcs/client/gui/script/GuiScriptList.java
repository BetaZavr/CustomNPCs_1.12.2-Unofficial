...
implements ICustomScrollListener {
	...
	public GuiScriptList(Map<String, Long> scripts, ScriptContainer container) {
		...
		for (String path : this.scripts.keySet()) {
			...
			if (path.indexOf("/")!=-1) {
				res = new ResourceLocation(path.substring(0, path.lastIndexOf("/")), path.substring(path.lastIndexOf("/")+1));
			}
			else {
	...
	public void buttonEvent(GuiNpcButton button) {
		...
		if (button.id == 2 && this.selected.hasSelected()) {
			try { file = this.selected.hoversTexts[this.selected.selected][0]; } catch (Exception e) { return; }
	...
	public void initGui() {
		...
		String p = ".../"+this.path;
		if (this.path.length() > 20) { p = "..."+this.path.substring(this.path.length()-20); }
			...
			if (temp.contains(file) || this.container.scripts.contains(file)) {
				...
				if (isBase) {
					...
					if (!this.path.isEmpty()) {
						if (foder.indexOf(this.path)!=0) { continue; }
						foder = foder.replace(this.path, "");
						if (foder.indexOf("/")==0) { foder = foder.substring(1); }
					}
					if (foder.indexOf("/")!=-1) { foder = foder.substring(0, foder.indexOf("/")); }
	...
	public void scrollClicked(int mouseX, int mouseY, int time, GuiCustomScroll scroll) {
		if (scroll.id==0) {
			if (scroll.getSelected().equals(this.back)) {
				if (this.path.lastIndexOf("/")==-1) { this.path = ""; }
				else { this.path = this.path.substring(0, this.path.lastIndexOf("/")); }
				this.base.selected = -1;
			}
	...
	public void scrollDoubleClicked(String select, GuiCustomScroll scroll) {
		String file = "";
		try { file = scroll.hoversTexts[scroll.selected][0]; } catch (Exception e) {}
