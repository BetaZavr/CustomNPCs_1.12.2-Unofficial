...
public class SubGuiScriptList extends SubGuiInterface implements ICustomScrollListener {
	...
	public void buttonEvent(@Nonnull GuiNpcButton button, int mouseButton) {
		...
		switch (button.getID()) {
			case 1: {
				...
				String file;
				try { file = base.getHoversTexts().get(base.getSelect()).get(0); } catch (Exception e) { return; }
			...
			case 2: {
				if (!base.hasSelected()) { return; }
				String file;
				try { file = selected.getHoversTexts().get(selected.getSelect()).get(0); } catch (Exception e) { return; }
	...
	public void initGui() {
		super.initGui();
		if (base == null) { base = new GuiCustomScroll(this, 0).setSize(140, 180); }
		base.guiLeft = guiLeft + 4;
		base.guiTop = guiTop + 14;
		addScroll(base);
		addLabel(new GuiNpcLabel(1, "script.availableScripts", guiLeft + 4, guiTop + 4));
		if (selected == null) { selected = new GuiCustomScroll(this, 1).setSize(140, 180); }
		...
		String p = ".../" + path;
		if (path.length() > 20) { p = "..." + path.substring(path.length() - 20); }
		...
		for (ResourceLocation res : data.keySet()) {
			...
			if (temp.contains(file) || container.scripts.contains(file)) {
				...
				if (isBase) {
					...
					if (!path.isEmpty()) {
						if (folder.indexOf(path) != 0) { continue; }
		...
		for (String key : ds.keySet()) {
			...
			suffixesBase.add("");
			listBase.add(key);
			if (hs.containsKey(key)) { htsB.put(i, Collections.singletonList(hs.get(key))); }
			i++;
		}
		for (String key : fs.keySet()) {
			...
			if (l < 0L) { l *= -1L; }
			String size = "" + l;
			if (l > 999) { size = Util.instance.getTextReducedNumber(l, false, false, true); }
			...
			if (hs.containsKey(key)) {
				...
				else { htsB.put(i, Collections.singletonList(hs.get(key))); }
			}
			i++;
		}
		...
		for (String key : ft.keySet()) {
			...
			if (hs.containsKey(key)) {
				...
				else { htsS.put(i, Collections.singletonList(hs.get(key))); }
			}
			i++;
		}
		base.setUnsortedList(listBase).setColors(colorsBase).setSuffixes(suffixesBase).setHoverTexts(htsB);
		selected.setUnsortedList(list).setColors(colors).setSuffixes(suffixes).setHoverTexts(htsS);
	...
	public void scrollDoubleClicked(String select, GuiCustomScroll scroll) {
		String file = "";
		try { file = scroll.getHoversTexts().get(scroll.getSelect()).get(0); }
