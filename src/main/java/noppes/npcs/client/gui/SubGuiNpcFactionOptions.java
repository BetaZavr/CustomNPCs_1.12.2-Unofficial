...
implements IScrollData, ICustomScrollListener, ITextfieldListener {
	...
	private void change(boolean isTake, int value) {
		...
		if (fo==null) {
			...
			if (value==0) {
				if ( this.options.remove(id)) { fo = null; }
			}
			else {
	...
	public void initGui() {
		...
		this.addLabel(new GuiNpcLabel(1, new TextComponentTranslation("faction.options").getFormattedText()+":", this.guiLeft + 5, this.guiTop + 4));
		...
		GuiNpcLabel lable = new GuiNpcLabel(2, new TextComponentTranslation("gui.settings").getFormattedText()+":", this.guiLeft + 130, this.guiTop + 4);
