...
implements ICustomScrollListener, ITextfieldListener, ISubGuiListener {
	...
	public void initGui() {
		...
		if (this.regions == null) { (this.regions = new GuiCustomScroll(this, 0)).setSize(110, 130); }
		...
		if (!selectReg.isEmpty()) { this.regions.setSelected(selectReg); }
		this.addScroll(this.regions);
		
		this.addLabel(new GuiNpcLabel(103, "gui.regions", this.guiLeft + 5, this.guiTop+4, 0xFF202020));

		if (this.points == null) { (this.points = new GuiCustomScroll(this, 1)).setSize(this.xSize-side-124, side/2); }
		...
		if (!selectP.isEmpty()) { this.points.setSelected(selectP); }
		...
		if (this.region!=null) { button.setTextColor(this.region.color); }
		...
		this.addTextField(textField);
		// ID 25 - Home X
		textField = new GuiNpcTextField(25, this, r0+39, this.guiTop+side+11, 35, 13, ""+(this.region!=null?this.region.homePos.getX() : ""));
		textField.enabled = this.region!=null && this.point!=null;
		this.addTextField(textField);
		// ID 26 - Home Y
		textField = new GuiNpcTextField(26, this, r0+77, this.guiTop+side+11, 35, 13, ""+(this.region!=null?this.region.homePos.getY() : ""));
		textField.enabled = this.region!=null && this.point!=null;
		this.addTextField(textField);
		// ID 27 - Home Z
		textField = new GuiNpcTextField(27, this, r0+115, this.guiTop+side+11, 35, 13, ""+(this.region!=null?this.region.homePos.getZ() : ""));
		textField.enabled = this.region!=null && this.point!=null;
		this.addTextField(textField);
		/** Lables */
		// ID 99 - Home Pos
		...
		// ID 100 - Min XZ Pos
		this.addLabel(new GuiNpcLabel(100, "MinXZ: ["+(this.region==null ? "0, 0" : this.region.getMinX()+","+this.region.getMinZ())+"]", r0+47, this.guiTop+6, 0xFF202020));
		// ID 101 - Max XZ Pos
		this.addLabel(new GuiNpcLabel(101, "MaxXZ: ["+(this.region==null ? "0, 0" : this.region.getMaxX()+","+this.region.getMaxZ())+"]", r0, this.guiTop+side-3, 0xFF202020));
		// ID 102 - Min/Max Y Pos
		String text = "Min/Max Y: ["+(this.region==null ? "0, 0" : this.region.y[0]+","+this.region.y[1])+"]";
		this.addLabel(new GuiNpcLabel(102, text, r0+side-this.mc.fontRenderer.getStringWidth(text)-1, this.guiTop+side-3, 0xFF202020));
		text = "(worldID: "+(this.region==null ? "N/A" : this.region.dimensionID)+")";
		this.addLabel(new GuiNpcLabel(105, text, r0+side-this.mc.fontRenderer.getStringWidth(text)-5, this.guiTop+4, 0xFF202020));
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (this.region.size()>3) { this.drawRegion(work, mu, mv, su, sv, sy, 0xFF0000FF); }
		...
		if (this.region.homePos!=null) {
			...
			if (hx<0) { hx = 0; } else if (hx>work) { hx = work; }
			if (hz<0) { hz = 0; } else if (hz>work) { hz = work; }
		...
		if (xp<mu) { xp = (int) mu;  color = 0xA0FFFFFF;} else if (xp>nu) { xp = (int) nu;  color = 0xA0FFFFFF;}
		hx = (int) ((xp-mu)*su);
		if (hx<0) { hx = 0; color = 0xF0FFFFFF; } else if (hx>work) { hx = work; color = 0xF0FFFFFF; }
		int zp = this.player.getPosition().getZ();
		if (zp<mv) { zp = (int) mv;  color = 0xF0FFFFFF;} else if (zp>nv) { zp = (int) nv;  color = 0xF0FFFFFF;}
		hz = (int) ((zp-mv)*sv);
		int d = (this.point!=null&&this.point.x==this.player.getPosition().getX()&&this.point.y==this.player.getPosition().getZ()?1:0) + 1;
		if (p!=null&&p.x==this.player.getPosition().getX()&&p.y==this.player.getPosition().getZ()) { d++; }
		if (c.getX()==this.player.getPosition().getX()&&c.getZ()==this.player.getPosition().getZ()) { d++; }
	...
	private void drawRegion(int work, double mu, double mv, double su, double sv, double sy, int color) {
		...
		for (int i=-1; i<this.region.points.size()-1; i++) {
			Point p0;
			if (i==-1) { p0 = this.region.points.get(this.region.points.size()-1); }
			else { p0 = this.region.points.get(i); }
		...
		switch (guibutton.id) {
			...
			case 10: {  // Up Point Pos
				...
				for (int pos : this.region.points.keySet()) {
					...
					if (p==this.point || (p.x==this.point.x && p.y==this.point.y)) { i = pos; break; }
				}
				int j=0;
				for (int pos : this.region.points.keySet()) {
					if (pos==i) { continue; }
					if (pos+1==i) { map.put(j++, this.point); }
					Point p = this.region.points.get(pos);
					map.put(j++, p);
				}
	...
	public void scrollClicked(int mouseX, int mouseY, int time, GuiCustomScroll scroll) {
		switch (scroll.id) {
			...
			case 1: { // Point List
				if (this.region==null || !this.dataPoints.containsValue(scroll.getSelected())) { return; }
	...
	public void unFocused(GuiNpcTextField textField) {
		...
		switch (textField.getId()) {
			case 16: { // X Point pos
				if (this.point==null || !textField.isInteger()) { return; }
