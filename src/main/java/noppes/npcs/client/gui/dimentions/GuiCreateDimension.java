...
extends GuiScreen
{
	...
	private void func_146314_g() {
		...
		while (this.field_146336_i.indexOf(" ")!=-1) { this.field_146336_i = this.field_146336_i.replace(" ", "_"); }
		...
		if (StringUtils.isEmpty(this.field_146336_i)) { this.field_146336_i = "World"; }
		this.field_146336_i = func_146317_a(this.mc.getSaveLoader(), this.field_146336_i);
	}

	private void updateDisplayState() {
		this.gameMode1 = new TextComponentTranslation("selectWorld.gameMode." + this.gameType + ".line1").getFormattedText();
		this.gameMode2 = new TextComponentTranslation("selectWorld.gameMode." + this.gameType + ".line2").getFormattedText();
		this.btnStructures.displayString = new TextComponentTranslation("selectWorld.mapFeatures").getFormattedText() + " ";
		if (this.generateStructures) { this.btnStructures.displayString = this.btnStructures.displayString + new TextComponentTranslation("options.on").getFormattedText(); }
		else { this.btnStructures.displayString = this.btnStructures.displayString + new TextComponentTranslation("options.off").getFormattedText(); }
		this.btnDimensionType.displayString = new TextComponentTranslation("selectWorld.mapType").getFormattedText() + " " + new TextComponentTranslation(WorldType.WORLD_TYPES[this.selectedIndex].getTranslationKey()).getFormattedText();
	}
	...
	public static String func_146317_a(ISaveFormat p_146317_0_, String p_146317_1_) {
		...
		for (int j = 0; j < i; ++j) {
			String s1 = astring[j];
			if (p_146317_1_.equalsIgnoreCase(s1)) { p_146317_1_ = "_" + p_146317_1_ + "_"; }
		}
		while (p_146317_0_.getWorldInfo(p_146317_1_) != null) { p_146317_1_ = p_146317_1_ + "-"; }
		return p_146317_1_;
	}
	...
	private void func_146315_i() { this.showMoreWorldOptions(!this.userInMoreOptions); }
	...
	private void showMoreWorldOptions(boolean toggle) {
		...
		if (WorldType.WORLD_TYPES[this.selectedIndex] == WorldType.DEBUG_ALL_BLOCK_STATES) {
			if (this.field_175300_s == null) { this.field_175300_s = this.gameType; }
		...
		this.updateDisplayState();
		if (this.userInMoreOptions) { this.btnMoreOptions.displayString = new TextComponentTranslation("gui.done").getFormattedText(); }
		else { this.btnMoreOptions.displayString = new TextComponentTranslation("dimensions.more.dimension.options").getFormattedText(); }
	}
	...
	protected void keyTyped(char typedChar, int keyCode) throws IOException {
		...
		if (this.dimensionNameTextField.isFocused() && !this.userInMoreOptions) {
			...
			while (this.dimentionName.indexOf(" ")!=-1) { this.dimentionName = this.dimentionName.replace(" ", "_"); }
		}
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		if (this.userInMoreOptions) { this.seedTextField.mouseClicked(mouseX, mouseY, mouseButton); }
		else { this.dimensionNameTextField.mouseClicked(mouseX, mouseY, mouseButton); }
	}

	@Override
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		this.drawDefaultBackground();
		this.drawCenteredString(this.fontRenderer, new TextComponentTranslation("dimensions.create").getFormattedText(), this.width / 2, 20, -1);
		if (this.userInMoreOptions) {
			this.drawString(this.fontRenderer, new TextComponentTranslation("selectWorld.enterSeed").getFormattedText(), this.width / 2 - 100, 47, -6250336);
			this.drawString(this.fontRenderer, new TextComponentTranslation("selectWorld.seedInfo").getFormattedText(), this.width / 2 - 100, 85, -6250336);
			if (this.btnStructures.visible) { this.drawString(this.fontRenderer, new TextComponentTranslation("selectWorld.mapFeatures.info").getFormattedText(), this.width / 2 - 150, 122, -6250336); }
			this.seedTextField.drawTextBox();
			if (WorldType.WORLD_TYPES[this.selectedIndex].hasInfoNotice()) { this.fontRenderer.drawSplitString(new TextComponentTranslation(WorldType.WORLD_TYPES[this.selectedIndex].getInfoTranslationKey()).getFormattedText(), this.btnDimensionType.x + 2, this.btnDimensionType.y + 22, this.btnDimensionType.getButtonWidth(), 10526880); }
		}
		else {
			this.drawString(this.fontRenderer, new TextComponentTranslation("dimensions.enter.name").getFormattedText(), this.width / 2 - 100, 47, -6250336);
			this.drawString(this.fontRenderer, new TextComponentTranslation("selectWorld.resultFolder").getFormattedText() + " " + this.field_146336_i, this.width / 2 - 100, 85, -6250336);
	...
	public void func_146318_a(WorldInfo p_146318_1_) {
		this.dimentionName = new TextComponentTranslation("selectWorld.newWorld.copyOf", new Object[] { p_146318_1_.getWorldName() }).getFormattedText();
		...
		if (p_146318_1_.isHardcoreModeEnabled()) { this.gameType = "hardcore"; }
		else if (p_146318_1_.getGameType().isSurvivalOrAdventure()) { this.gameType = "survival"; }
		else if (p_146318_1_.getGameType().isCreative()) { this.gameType = "creative"; }
	}
