...
extends GuiScreen {
	...
	public String getPreset() { return this.theFlatGeneratorInfo.toString(); }

	public void setPreset(String preset) { this.theFlatGeneratorInfo = FlatGeneratorInfo.createFlatGeneratorFromString(preset); }
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		if (button.id == 1) { this.mc.displayGuiScreen(this.createDimensionGui); }
		else if (button.id == 0) {
			this.createDimensionGui.chunkProviderSettingsJson = this.getPreset();
			this.mc.displayGuiScreen(this.createDimensionGui);
		}
		else if (button.id == 5) { this.mc.displayGuiScreen(new GuiFlatDimensionPresets(this)); }
	...
	private boolean hasSelectedLayer() { return this.createFlatWorldListSlotGui.selectedLayer > -1 && this.createFlatWorldListSlotGui.selectedLayer < this.theFlatGeneratorInfo.getFlatLayers().size(); }
	...
	class Details extends GuiSlot {
		...
		public Details() { super(GuiCreateFlatDimension.this.mc, GuiCreateFlatDimension.this.width, GuiCreateFlatDimension.this.height, 43, GuiCreateFlatDimension.this.height - 60, 24); }
		...
		protected void drawSlot(int entryID, int insideLeft, int yPos, int insideSlotHeight, int mouseXIn, int mouseYIn, float partialTicks) {
			...
			String s1;
			if (entryID == 0) { s1 = I18n.format("createWorld.customize.flat.layer.top", new Object[] {Integer.valueOf(flatlayerinfo.getLayerCount())}); }
			else if (entryID == GuiCreateFlatDimension.this.theFlatGeneratorInfo.getFlatLayers().size() - 1) { s1 = I18n.format("createWorld.customize.flat.layer.bottom", new Object[] {Integer.valueOf(flatlayerinfo.getLayerCount())}); }
			else { s1 = I18n.format("createWorld.customize.flat.layer", new Object[] {Integer.valueOf(flatlayerinfo.getLayerCount())}); }
		...
		protected int getScrollBarX() { return this.width - 70; }
	}
