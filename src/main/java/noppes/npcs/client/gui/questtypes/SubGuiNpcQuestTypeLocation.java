...
public class SubGuiNpcQuestTypeLocation extends SubGuiInterface implements ITextfieldListener {

	protected final QuestObjective task;
	protected final Map<Integer, Integer> dataDimIDs = new HashMap<>();

	public SubGuiNpcQuestTypeLocation(EntityNPCInterface npc, QuestObjective taskObj, GuiScreen gui) {
		super(0, npc);
	...
	public void initGui() {
		...
		addTextField(new GuiNpcTextField(0, this, x, y += 12, 244, 18, task.getTargetName())
		...
		addTextField(new GuiNpcTextField(12, this, x + 135, y, 40, 14, "" + task.pos.getZ())
		...
		addTextField(new GuiNpcTextField(14, this, x + 199, y, 45, 14, "" + task.rangeCompass)
		...
		addTextField(new GuiNpcTextField(15, this, x + 89, y, 155, 14, task.entityName)
				.setHoverText(new TextComponentTranslation("quest.hover.compass.entity").appendSibling(compass).getFormattedText()));
		// set player pos
		addButton(new GuiNpcButton(10, x + 185, y += 19, 60, 20, "gui.set")
				.setHoverText(new TextComponentTranslation("quest.hover.compass.set").appendSibling(compass).getFormattedText()));
		// tp
		addButton(new GuiNpcButton(11, x + 163, y, 20, 20, "TP")
				.setHoverText("hover.teleport"));
		// mini map point
		addButton(new GuiNpcCheckBox(5, x, y, 160, 16, "quest.set.minimap.point", null, task.isSetPointOnMiniMap())
	...
	public void save() {
		...
		for (QuestObjective taskObj : NoppesUtilServer.getEditingQuest(player).questInterface.tasks) {
			if (taskObj == task || taskObj.getEnumType() != EnumQuestTask.LOCATION) { continue; }
