...
extends GuiContainerNPCInterface2 {
	...
	public void initGui() {
		...
		x += 34;
		y -= 33;
		this.addTextField(new GuiNpcTextField(3, (GuiScreen) this, this.fontRenderer, x, y, 286, 16, this.role.dialogHire));
		this.addTextField(new GuiNpcTextField(4, (GuiScreen) this, this.fontRenderer, x, y += 19, 286, 16, this.role.dialogFarewell));
		this.addTextField(new GuiNpcTextField(5, (GuiScreen) this, this.fontRenderer, x, y += 19, 286, 16, this.role.dialogFired));
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (this.getTextField(3)!=null && this.getTextField(3).isMouseOver()) {
			...
			this.setHoverText(new TextComponentTranslation("follower.hover.soulstone", new TextComponentTranslation("item.npcsoulstoneempty.name").getFormattedText()).getFormattedText());
			...
			for (int i = 0; i < 3; i++) {
				if (this.getTextField(i)!=null && this.getTextField(i).isMouseOver()) {
					this.setHoverText(new TextComponentTranslation("follower.hover.days", "" + (i + 1)).getFormattedText());
				}
			}
		}
	}
	...
	public void save() {
		...
		for (int i = 0; i < 3; ++i) {
			int days = 1;
			if (!this.getTextField(i).isEmpty() && this.getTextField(i).isInteger()) { days = this.getTextField(i).getInteger(); }
			if (days <= 0) { days = 1; }
			map.put(i, days);
		}
		...
		if (this.role.rentalMoney > 0) {
			int days = 1;
			if (!this.getTextField(7).isEmpty() && this.getTextField(7).isInteger()) { days = this.getTextField(7).getInteger(); }
			if (days <= 0) { days = 1; }
			map.put(3, days);
		}
		this.role.rates = map;
		...
		if (this.role.inventory.getSizeInventory() != size) { this.role.inventory = new NpcMiscInventory(size); }
