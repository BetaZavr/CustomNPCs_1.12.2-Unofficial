...
implements IGuiData, ICustomScrollListener, ITextfieldListener {
	...
	public void initGui() {
		...
		} else { this.aliveScroll.selected = -1; }
		...
		} else { this.deadScroll.selected = -1; }
		...
		this.addButton(new GuiNpcButton(8, this.guiLeft + 63, this.guiTop + 190, 55, 20, new String[] { "spawner.one", "spawner.all", "spawner.random" }, this.job.getSpawnType(false)));
	...
	public void buttonEvent(GuiNpcButton button) {
		switch(button.id) {
			...
			case 4: { // up alive
				if (this.isDead || this.slot<1) { return; }
			...
			case 5: { // down alive
				if (this.isDead || this.slot>=this.job.size(this.isDead)) { return; }
	...
	public void closeSubGui(SubGuiInterface gui) {
		...
		if (this.slot<0) { this.slot = (this.isDead ? this.deadScroll : this.aliveScroll).getList().size(); }
	...
	public void setGuiData(NBTTagCompound compound) {
		if (compound.hasKey("NPCData", 10)) { // Entity set
			this.selectNpc = (EntityLivingBase) EntityList.createEntityFromNBT(compound.getCompoundTag("NPCData"), this.player.world);
			return;
		}
		// Setts
		char chr = ((char) 167);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		for (int t=0; t<6; t++) {
			if (mouseX >= this.guiLeft+(t<3?52:227)+(t%3*45) && mouseY >= this.guiTop+161 && mouseX < this.guiLeft+(t<3?87:262)+(t%3*45) && mouseY < this.guiTop+176) {
				this.setHoverText(new TextComponentTranslation("spawner.hover.axis.offset", (t%3==0 ? "X" : t%3==1 ? "Y" : "Z")).appendSibling(new TextComponentTranslation("spawner.hover.sp."+((int) Math.floor(t/3)))).getFormattedText());
				return;
			}
		}
