...
extends GuiNpcButton {
	...
	public void drawButton(Minecraft minecraft, int i, int j, float partialTicks) {
		...
		if (this.height == 20) {
			...
			this.drawTexturedModalRect(this.x, this.y, 0, k * 20, this.width, this.height + (!this.active && this.hovered ? 1 : 0) - 4);
			this.drawTexturedModalRect(this.x, this.y + this.height - 4, 0, (k + 1) * 20 - 4, this.width, 4 + (!this.active && this.hovered ? 1 : 0));
		}
		...
		this.drawCenteredString(fontrenderer, this.displayString, this.x + this.width / 2, this.y + this.offsetYtext + (this.height - 8) / 2, l);
