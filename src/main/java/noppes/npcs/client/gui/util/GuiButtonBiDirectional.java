...
extends GuiNpcButton {
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
		...
		this.drawHorizontalLine(this.x + 7, this.x + this.width - 12, this.y, 0xFF000000); // New
		this.drawHorizontalLine(this.x + 7, this.x + this.width - 12, this.y + this.height - 1, 0xFF000000); // New
		...
		this.drawTexturedModalRect(this.x + this.width - 11, this.y, 11,
				((this.hovered && !hoverL) || hoverR) ? 40 : 20, 11, 20);
		int l = this.color;
		...
		if (this.hovered) {
			text = new String(Character.toChars(0x00A7)) + "n" + text;
		}
