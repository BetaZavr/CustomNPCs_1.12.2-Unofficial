...
implements ISubGuiListener {
	...
	public void drawToolTip(List<String> stringList, int x, int y) {
		...
		for (String str : stringList) {
			if (name != null) {
				if (name.isEmpty()) { name = str.toLowerCase(); }
				else {
					newToolTip.add(((char) 167) + "e" + new TextComponentTranslation("property."+AdditionalMethods.instance.deleteColor(name)+".hover").getFormattedText());
					name = null;
					continue;
				}
			}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		if (subGui != null) {
			...
			if (subGui.object instanceof ColorEntry) { drawCenteredString(this.fontRenderer, ((ColorEntry) subGui.object).getName(), this.width / 2, 18, 0xFFA0A0A0); }
			...
			if (undoHoverChecker.checkHover(mouseX, mouseY)) { drawToolTip(Arrays.asList(I18n.format("fml.configgui.tooltip.undoAll").split("\n")), mouseX, mouseY); }
			if (resetHoverChecker.checkHover(mouseX, mouseY)) { drawToolTip(Arrays.asList(I18n.format("fml.configgui.tooltip.resetAll").split("\n")), mouseX, mouseY); }
			if (checkBoxHoverChecker.checkHover(mouseX, mouseY)) { drawToolTip(Arrays.asList(I18n.format("fml.configgui.tooltip.applyGlobally").split("\n")), mouseX, mouseY); }
			return;
		}
	...
	public static class CustomGuiEditArray extends GuiEditArray implements ISubGuiListener {
		...
		public CustomGuiEditArray(GuiScreen parentScreen, IConfigElement configElement, int slotIndex, Object[] currentValues, boolean enabled) { super(parentScreen, configElement, slotIndex, currentValues, enabled); }

		@Override
		protected GuiEditArrayEntries createEditArrayEntries() { return new CustomGuiEditArrayEntries(this, this.mc, this.configElement, this.beforeValues, this.currentValues); }
		...
		public void drawScreen(int mouseX, int mouseY, float partialTicks) {
			if (subGui != null) {
		        ...
		        if (this.titleLine2 != null) { this.drawCenteredString(this.fontRenderer, this.titleLine2, this.width / 2, 18, 16777215); }
		        if (this.titleLine3 != null) { this.drawCenteredString(this.fontRenderer, this.titleLine3, this.width / 2, 28, 16777215); }
				return;
			}
			super.drawScreen(mouseX, mouseY, partialTicks);
		}
