...
public class NpcEntityDamageSource extends EntityDamageSource implements IEntityDamageSource {
	...
	public ITextComponent getDeathMessage(EntityLivingBase entity) {
		...
		if (!this.deadMessage.isEmpty()) {
			return new TextComponentTranslation(this.deadMessage,
					new Object[] { entity.getDisplayName(), entitySourceName,
							new TextComponentTranslation(this.damageType).getFormattedText(),
							stack.getTextComponent() });
		}
		String s = "death.attack." + this.damageType;
		String s1 = s + ".item";
		ITextComponent ts1 = new TextComponentTranslation(s1,
				new Object[] { entity.getDisplayName(), entitySourceName, stack.getTextComponent() });
		return !stack.isEmpty() && stack.hasDisplayName() && ts1.getFormattedText().equals(s1) ? ts1
				: new TextComponentTranslation(s, new Object[] { entity.getDisplayName(), entitySourceName });
	}

	@Override
	public IEntity<?> getIImmediateSource() {
		return this.indirectEntity == null ? null : NpcAPI.Instance().getIEntity(this.indirectEntity);
	}
	...
	public IEntity<?> getITrueSource() {
		return this.damageSourceEntity == null ? null : NpcAPI.Instance().getIEntity(this.damageSourceEntity);
	}
