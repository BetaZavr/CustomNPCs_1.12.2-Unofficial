...
This code is published with the permission of the author under the terms of **[ Creative Commons Attribution-Non Commercial 3.0 Unported license ](http://creativecommons.org/licenses/by-nc/3.0/)** for CustomNPC .
...
16. Sub -GUI for selecting sounds:
...
### Dialogues:
1. The dialogue response system has been completely redesigned:
* you can add an icon;
* you can set options for the next dialog through accessibility conditions;
2. the dialog can be shown more than once (display as when typing text);
3. you can add an image to the end of the dialog;
4. The GUI for player communication with NPCs has been changed (see above in GUI);
5. more dialog settings;

### Factions:
* Added flag selection. Flags can be installed on banners and the vanilla shield;
* Added a setting for selecting friendly factions. Used for defense during attacks;

### Natural spawns :
* spawn mechanics have been finalized;
* more settings for determining natural spawn;

### Scripts:
Currently, there's no database of all mod APIs for the available versions. Maybe I'll get around to it someday.
...
10. The mouse-held item has been added to the slot click event in your GUI **CustomGuiEvent.SlotEvent**;
